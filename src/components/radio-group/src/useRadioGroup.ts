...
export function useRadioGroup<T extends ObjectLike>(
  ...
  const { createProps, globalProps } = useComponentProps(props as RadioGroupProps, 'form', { propNames, globalPropNames, handleProps })
  ...
  return {
    render: () => h(RadioGroup, {
      ...
      props: createProps(),
      on
    }, (props as RadioGroupProps).options.map((item) => h(Radio, {
      props: {
        label: item.value,
        name: item.name,
